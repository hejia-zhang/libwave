...
tensorflow::Status ReadEntireFile(tensorflow::Env *env, const std::string &file_path, tensorflow::Tensor &output) {
  ...
  if (data.size() != file_size) {
    return tensorflow::errors::DataLoss("Truncated read of '",
                                        file_path,
                                        "' expected ",
                                        file_size,
                                        " got ",
...
int ObjectDetectThread::Detect(const ImageFrame &imgFrame) {
  std::string label_path = "/home/hejia/PycharmProjects/tflearn/auv_data/auv_label_map.pbtxt";
  ...
  std::string graph_path = "/home/hejia/PycharmProjects/tflearn/models/transfer_to_auv/frozen_inference_graph.pb";
  ...
  std::string image_path = "/home/hejia/PycharmProjects/tflearn/images/image4.png";
  ...
  LOG(INFO) << "size: " << outputs.size() << ", image_width: " << image_width << ", image_height: " << image_height
            << std::endl;
  ...
  for (size_t i = 0; i < num_detections(0) && i < 20; i++) {
    if (scores(i) > 0.5) {
      LOG(INFO) << i << ", score: " << scores(i) * 100 << "%" << ", class: " << classes(i) << ", box" << i << ": "
                << "[ymin, xmin, ymax, xmax]: [" << boxes(0, i, 0)
  ...
  for (;;) {
    ...
    try {
      ...
      for (size_t i = 0; i < num_detections(0) && i < 20; i++) {
        ...
        if (scores(i) > 0.7) {
          m_logger.information(Poco::format("score: %d, class: %d box: [%d, %d, %d, %d]",
                                            int(scores(i) * 100),
                                            int(classes(i)),
                                            ymin,
                                            xmin,
          ...
          cv::rectangle(pWorkNf->m_frame.m_img,
